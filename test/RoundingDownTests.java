...
public class RoundingDownTests {
    ...
    @Test
    void testMonetaryRoundingDown() {
        LocalDateTime testTime = LocalDateTime.now();
        InsuranceCompany company = new InsuranceCompany(testTime);
        Person policyholder = new Person("8352244395");
        Person beneficiary = new Person("8303231992");

        // Test case 1: Vehicle with value 5 should have coverage of 2 (not 2.5)
        Vehicle vehicleOddValue = new Vehicle("ABC123E", 5);
        SingleVehicleContract contractOddValue = company.insureVehicle(
                "C001", beneficiary, policyholder, 200,
                PremiumPaymentFrequency.MONTHLY, vehicleOddValue
        );
        assertEquals(2, contractOddValue.getCoverageAmount());

        // Test case 2: Vehicle with value 4 should have coverage of 2
        Vehicle vehicleEvenValue = new Vehicle("XYZ456F", 4);
        SingleVehicleContract contractEvenValue = company.insureVehicle(
                "C002", beneficiary, policyholder, 200,
                PremiumPaymentFrequency.MONTHLY, vehicleEvenValue
        );
        assertEquals(2, contractEvenValue.getCoverageAmount());

        // Test case 3: Process claim with 70% damage threshold
        Vehicle vehicle = new Vehicle("DEF789G", 5);
        SingleVehicleContract contract = company.insureVehicle(
                "C003", beneficiary, policyholder, 200,
                PremiumPaymentFrequency.MONTHLY, vehicle
        );

        // Damage of 3 (not 3.5) should not deactivate the contract (< 70%)
        company.processClaim(contract, 3);
        assertTrue(contract.isActive());

        // Test case 4: Travel contract claim distribution
        Person person1 = new Person("6503090803");
        Person person2 = new Person("6502268399");
        Person person3 = new Person("6502262624");
        Set<Person> insuredPersons = Set.of(person1, person2, person3);

        // Create travel contract with odd coverage amount
        TravelContract travelContract = company.insurePersons(
                "T001", policyholder, 30,
                PremiumPaymentFrequency.MONTHLY, insuredPersons
        );

        // Set coverage to 5 for testing
        travelContract.setCoverageAmount(5);

        // Process claim for all persons
        int initialPerson1Balance = person1.getPaidOutAmount();
        company.processClaim(travelContract, insuredPersons);

        // Each person should get 1 (5/3 = 1.66... rounded down to 1)
        assertEquals(initialPerson1Balance + 1, person1.getPaidOutAmount());
    }
}